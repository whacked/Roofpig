...
describe("Alg", function() {
  ...
  describe("#constructor", function() {
    it("reads move strings", function() {
      expect(new Alg("U F2 D' LZ").to_s()).to.equal("U F2 D' LZ");
      expect(new Alg(" U F2  D' LZ  ").to_s()).to.equal("U F2 D' LZ");
      return expect(new Alg("F D+U'").to_s()).to.equal("F (D U')");
    });

    return it("handles empty alg", function() {
      expect(new Alg("").at_start()).to.be.true;
      return expect(new Alg("").at_end()).to.be.true;
    });
  });

  it("keeps track of moves", function() {
    const alg = new Alg('F D');
    ...
    alg.next_move();
    expect(alg.at_start()).to.be.false;
    return expect(alg.at_end()).to.be.true;
  });

  it("doesn't need a Dom", function() {
    const alone_alg = new Alg("U F2 D' LZ", null);
    return expect(alone_alg.to_s()).to.equal("U F2 D' LZ");
  });

  it("builds the right Move objects", function() {
    move_should_be(new Alg("F2").moves[0], Layer.F, 2);

    move_should_be(new Alg("R<").moves[0], Layer.R, -1, true);

    const compound = new Alg("L'+F>>").moves[0];
    ...
    const wide_u = new Alg("Uw2").moves[0];
    ...
    const wide_u2 = new Alg("u2").moves[0];
    ...
    return move_should_be(wide_u2.moves[1], Layer.E, -2);
  });

  describe("#_count_text", function() {
    const world = null;

    it("ignoring rotations", function() {
      const alg = new Alg("F L> D'+U R+R>", world, new Config("").algdisplay);
      ...
      alg.next_move();
      return expect(alg._count_text()).to.equal('4/4');
    });

    return it("counting rotations", function() {
      const alg = new Alg("F L> D'+U R+R>", world, new Config("algdisplay=rotations").algdisplay);
      ...
      alg.next_move();
      return expect(alg._count_text()).to.equal('6/6');
    });
  });
  ...
  describe("#display_text", function() {
    ...
    it("separates past and future moves", function() {
      const alg = new Alg("F R> U+D' L2 R' LZ D+D>", world, new Config("").algdisplay);
      ...
      alg.next_move();
      alg.next_move();
      return expect(alg.display_text()).to.deep.equal({past:"F U+D'", future: "L2 R' L2 D"});
    });

    return it("uses algdisplay", function() {
      let alg = new Alg("F R> U2+D' F<< LZ D+D>", world, new Config("algdisplay=Z").algdisplay);
      ...
      return expect(alg.display_text().future).to.equal("F R> U2+D' F<< L2 D+D>");
    });
  });

  it("pov_from", function() {
    ...
    return expect(Alg.pov_from("MZ").map).to.deep.equal({U: 'D', D: 'U', L: 'L', R: 'R', F: 'B', B: 'F'});
  });
  ...
  it("unhand", function() {
    ...
    // weird cases
    return expect(new Alg("F y L2", null, "").unhand()).to.equal("F F2");
  });


  return move_should_be = function(move, layer, turns, is_rotation) {
    if (is_rotation == null) { is_rotation = false; }
    expect(move.layer, move.to_s()).to.equal(layer);
    expect(move.turns, move.to_s()).to.equal(turns);
    return expect(move.is_rotation, move.to_s()).to.equal(is_rotation);
  };
});
